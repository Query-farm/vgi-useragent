//! The `useragent` VGI worker (library).
//!
...
//! the `useragent-wasm` crate (the browser build, which serves the same
...
/// The catalog name DuckDB sees in `ATTACH 'useragent' (TYPE vgi, …)`. Defaults
/// to `useragent`, but honors an explicit override so a test harness can rename
