...
#
# Dispatch the single vgi-useragent image into one of its transports:
...
#
# The worker is stateless (embedded uap-core regex database), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec useragent-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec useragent-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec useragent-worker "$@"
    ;;
  *)
    exec "$@"
    ;;
